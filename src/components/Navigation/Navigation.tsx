...
export const Navigation = ({ variant = 'home' }: NavigationProps): JSX.Element => {
  ...
  return (
    <>
      <header role="banner" className="fixed top-0 left-0 right-0 z-[100] flex flex-col w-full items-center justify-center gap-[11px] bg-[linear-gradient(0deg,rgba(0,0,0,0.75)_0%,rgba(0,0,0,0.75)_100%),url(https://c.animaapp.com/mjyhvu36aqUy0x/img/header.png)] bg-cover bg-center py-4 border-b-[0.5px] border-[#51b0ff]">
        {/* Desktop Navigation (1280px+) */}
        <div className="hidden xl:flex w-full max-w-[1128px] mx-auto px-4">
          <nav role="navigation" aria-label="Main navigation" className="flex w-full items-center justify-between translate-y-[-1rem] animate-fade-in opacity-0">
            <button
        ...
        <div className="hidden md:flex xl:hidden w-full max-w-[694px] mx-auto px-4">
          <nav className="flex w-full items-center justify-between translate-y-[-1rem] animate-fade-in opacity-0">
            <button
              onClick={handleLogoClick}
              className="cursor-pointer bg-transparent border-0 p-0 focus:outline-2 focus:outline-offset-2 focus:outline-white"
              aria-label={variant === 'legal' ? 'Return to homepage' : 'Return to top of page'}
        ...
        <div className="flex md:hidden w-full max-w-full mx-auto px-4">
          <nav className="flex w-full items-center justify-between">
            ...
            <button
              onClick={toggleMenu}
              className="relative w-8 h-8 flex items-center justify-center text-white hover:opacity-80 transition-opacity focus:outline-2 focus:outline-offset-2 focus:outline-white z-[10001]"
        ...
        {/* Mobile Menu Overlay */}
        <div
          className={`fixed inset-0 z-[10000] bg-black/95 md:hidden transition-opacity duration-300 ${
